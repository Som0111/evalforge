...
pytest tests/                                    # 85 tests, no API key needed (judge calls are mocked)
...
- **Free tier.** Live at https://evalforge-cfyw.onrender.com ([/docs](https://evalforge-cfyw.onrender.com/docs), [/demo](https://evalforge-cfyw.onrender.com/demo)). The Render free instance spins down when idle; the first request after a while can take ~50s to wake it up. `/evaluate` requires the deployment's `X-API-Key`, generated by Render.
